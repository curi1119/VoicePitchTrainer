...
ピッチラボ（Pitch Lab）。マイク入力を YIN 法でピッチ検出し、チューナー・鍵盤・単音発声・跳躍発声・音階練習の各モードで音程を扱う Web アプリ。
...
main へ push → `.github/workflows/deploy.yml` が GitHub Pages へ自動デプロイ（`pitch-trainer/` 配下の変更時のみ）。
